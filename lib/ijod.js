...
var friendUtils = require('friends-utilities');
var partition = require('partition');
var ijodEvent = require('ijod-event');

// we use redis just to signal administrative deletes into a queue, in db1
var rclient = require("redis").createClient(lconfig.taskman.redis.port,
                                            lconfig.taskman.redis.host);
rclient.select(1, function() {});
  ...
  partition.readFrom(base, function(parts){
    ...
    async.forEach(parts, function(part, cbParts){
        ...
        if(rows) rows.forEach(function(row){
          if(ndx[row.idr]) { // if this idr exists, see if we have to replace it
            if(part.priority > ndx[row.idr].pri) {
              ...
              // DSS: Disabling this call; move to pod arch should invalidate need for it
              //rclient.sadd("dirty",row.idr.toLowerCase()+'_'+partition.getPart(base)); // flag for background cleanup
            }
            return;
          }
      ...
      pNew.dal.query("select hex(idr) as idr from "+pNew.table+" where idr in ("+ins+")", [], function(err, rows){
        if(rows) rows.forEach(function(row){
          logger.debug("deleting old invalid instance",row.idr,ret[ndx[row.idr].at].idr);
          ret[ndx[row.idr].at].del = true;
          // DSS: Disabling this call; move to pod arch should invalidate need for it
          //rclient.sadd("dirty",row.idr.toLowerCase()+'_'+partition.getPart(base)); // flag for background cleanup
        });
        // remove any deleted ones
        ret = ret.filter(function(r){ return !r.del; });
        logger.debug("returning items",ret.length);
        cbDone(null, ret);
      });
    });
  });
...
//  instruments.increment("ijod.partitions."+(partition.getPart(id)||"none")+".mod").send();
  ...
  partition[source](id, function(parts){
    ...
    async.forEach(parts, function(part, cbParts){
      part.dal.query(sql.replace(/T4BL3/g, part.table), binds.slice(0), function(err, rows){
...
exports.getPars = function(basePath, options, cbDone) {
  ...
  ijodEvent.publish("getPars", {base: basePath, options: options});
...
exports.getOnePars = function (targetIdr, cat, cbDone) {
  ...
  if (cat) {
    entryInClause = "x'" + friendUtils.parallelCategory(hash, cat) + "'";
  } else {
    entryInClause = Object.keys(friendUtils.parallelCategories()).map(function (cat) {
      return "x'" + friendUtils.parallelCategory(hash, cat) + "'";
    }).join(",");
  }

  var sql = "SELECT lpad(hex(par),32,'0') as par, path, hex(idr) as idr " +
    "FROM T4BL3 WHERE idr in (" + entryInClause + ")";

  ijodEvent.publish("getOnePars", { idr: targetIdr, cat: cat });
...
exports.setOneCat = function (id, cat, options, cbDone) {
  ...
  if (options.pars) {
    options.pars.forEach(function (par) {
  ...
  // 10yrs into the future, as a way to make sure they don't show up as a normal
  // entry
  var at = Date.now() + (10 * 365 * 24 * 3600 * 1000);

  var binds = [
    idr.baseHash(id), at, idr.hash(id),
    friendUtils.parallelCategory(idr.hash(id), cat),
  ...
  ijodEvent.publish("setOneCat", { idr: id, cat: cat, options: options });
...
function parget(entry) {
  ...
  if (dparts) {
    Object.keys(dparts).forEach(function (id) {
      if (dparts[id].author) {
        author = id.toString();
        return;
      }
...
IJOD.prototype.addData = function(arg, callback) {
  ...
  var q = qget(arg);
  var buf = qix.buf(q);
  // also be more sensitive if there's parallels (friends) being indexed
  if(parallels.length > 0) buf = qix.buf(q, true, buf);
  ...
  arg.store = {path:self.path, len:gzdata.length, offset:offset, hash:hash};
...
exports.getOne = function(targetIdr, callback) {
  ...
  ijodEvent.publish("getOne", { idr: targetIdr });
  var hash = partition.getHash(targetIdr);
...
exports.delOne = function(targetIdr, callback) {
  ...
  ijodEvent.publish("delOne", { idr: targetIdr });
...
exports.getRange = function(basePath, range, cbEach, cbDone) {
  ...
  ijodEvent.publish("getRange", { base: basePath, range: range });
...
exports.batchSmartAdd = function(entries, callback) {
  ...
  ijodEvent.publish("hashCheck", { base: basePath, idrs: entries.map(function(entry){
    return idr.hash(entry.idr);
  }) });
...
exports.getBounds = function(basePath, range, cbDone) {
  ...
  sql += parq(range.participants);
  sql += ") AS sq1";
  ijodEvent.publish("getBounds", { base: basePath, range: range });
...
exports.getTardis = function(basePath, range, cbDone) {
  ...
  sql += qq(range.q);
  sql += parq(range.participants);
  ijodEvent.publish("getTardis", { base: basePath, range: range });
...
exports.pump = function(arg, cbDone) {
  ...
  var timings = {};
  var start = Date.now();
  async.forEachSeries(Object.keys(bases), function(base, cb) {
