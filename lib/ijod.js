...
var friendsUtil = require('friends-utilities');
...
exports.getOnePars = function (targetIdr, cat, cbDone) {
  ...
  if (cat) {
    entryInClause = "x'" + friendsUtil.parallelCategory(hash, cat) + "'";
  } else {
    entryInClause = Object.keys(friendsUtil.parallelCategories())
      .map(function (cat) {
        return "x'" + friendsUtil.parallelCategory(hash, cat) + "'";
      }).join(',');
  }
...
exports.setOneCat = function (id, cat, options, cbDone) {
  ...
  var binds = [
    ...
    friendsUtil.parallelCategory(idr.hash(id), cat),
