...
exports.contactSync = function(type, pi, cbDone) {
  ...
  console.log("function : getOne arguments");
  console.log(arg);
  ...
  getOne(client, arg, function(err, userIDs) {
    ...
    /* twitter client deletes the token , so set it again */
    arg.token = client.token;
    getOne(client, arg, function(err, userIDs) {
      if (err) return cbDone(err);
      ...
      /* cross section of friends and followers */

      var followerfriends = intersect_safe([followers, friends]);


      console.log("_____________________");

      console.log("function : get Users");
      console.log(followerfriends);
      getUsers(client, followerfriends, function(err, users) {
        if (err) return cbDone(err);
