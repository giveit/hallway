  ...
  getPage(arg.uri, function(err, friends) {
    ...
    getPage(friendsURI, function(err, friendData){
      ...
      if (!err) {
    	...
    	//  console.log(friendData[key]);
          cbEach(friendData[key]);
        });
      }

      if (friends.paging && friends.paging.next) {
        arg.uri = friends.paging.next;
        return exports.getFriends(arg, cbEach, cbDone);
      } else {
        return cbDone();
      }
    });
...
exports.getAdministeredPages = function(arg, cbEach, cbDone) {
  arg.uri = exports.apiUrl(arg, '/me/accounts', {type: 'page'});
  return getPages(arg, cbEach, cbDone);
};

function processPageLikes(pages, arg, cbDone){
  if (pages.length === 0) {
    cbDone('pages.length === 0');
  } else {
    var pageIDs = [];
...
exports.getPageLikes = function(arg, cbDone) {
  ...
  exports.getFQL(arg, function(err, pages){
    ...
    if (pages && pages.length === 0) {
      return cbDone(null, arg.pageLikes, arg.newSince);
    }

		processPageLikes(pages, arg, function(err, arg) {
      if (err) return cbDone(err);
      arg.offset += SMALL_PAGE_SIZE;
      process.nextTick(exports.getPageLikes.bind(null, arg, cbDone));
		});
	});
};
