...
var reverseParallelTypes = []; // built during startup
...
// find next one just:
// $ grep 'exports.ptype' lib/services/*/map.js
exports.parallelType = function (type) {
  if (typeof type === 'number') return reverseParallelTypes[type];

  if (parallelTypes[type]) return parallelTypes[type];

  return 0;
};

// dynamically pull in any app maps, this shouldn't be called too often but
// regularly (at start of pipeline right now)
...
exports.loadcheck = function (service, callback) {
  // pass-through system loaded ones
  if (maps[service] && maps[service]._system) return callback();
  ...
  ijod.getOne(idr, function (err, entry) {
    ...
    // hasn't changed
    if (maps[service] && maps[service]._loaded === entry.at) return callback();

    // time to load a new one, someday we'll need to handle dynamic functions,
    // pumps, etc, starting KISS
...
// handy way to make sure there's a default mapping between a context and type,
// used primarily for app saved data
  ...
  logs: {
    text: function (data) {
      ...
      data.forEach(function (item) {
        if (item.from) {
          keys[item.from.split('.').map(function (y) {
...
exports.load = function () {
  ...
  services.forEach(function (service) {
    ...
    try {
      ...
      if (maps[service].ptype) parallelTypes[service] = maps[service].ptype;
    } catch (E) {
  ...
  Object.keys(parallelTypes).forEach(function (type) {
    if (reverseParallelTypes[parallelTypes[type]]) {
      // XXX: This seems like it could cause corruption... Should we exit?
      logger.warn("conflicting type!", type, parallelTypes[type],
        reverseParallelTypes[parallelTypes[type]]);
    }

    reverseParallelTypes[parallelTypes[type]] = type;
  });
};
