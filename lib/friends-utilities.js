...
var parallelType = exports.parallelType = function (par) {
...
exports.status = function (pars) {
  ...
  pars.forEach(function (par) {
    if (parallelType(par) !== 'status') return;
...
var parallelFromBytes = exports.parallelFromBytes = function (parts) {
...
var selectParallel = exports.selectParallel = function (pars, type) {
  var ret = parallelFromBytes([type, 0]); // default blank

  if (!pars) pars = [];

  pars.forEach(function (par) {
    if (parallelType(par) === 'status') ret = par;
  });
...
// increment a par
var incrementParallel = exports.incrementParallel = function (par, val) {
  if (!val) return par;
...
var parUpdateStatus = exports.parUpdateStatus = function (pars, status, value) {
  var spar = parallelFromBytes(['status', 0]); // default blank
  var ret = [];
  ...
  pars.forEach(function (par) {
    ...
    if (parallelType(par) === 'status') spar = par;
    else ret.push(par);
  });
...
var pid2par = exports.pid2par = function (pid) {
  ...
  return parallelFromBytes([parts[1], str2num(parts[0], 3)]);
};
...
exports.tocmap = function (sort) {
  return 'abcdefghijklmnopqrstuvwxyz'.split('').map(function (c) {
    return {
      c: c,
      v: parseInt(parallelFromBytes([sort, str2sort(c + 'aa'), 0]), 16)
    };
  });
};
  ...
  async.forEach(bases, function (base, cbBase) {
    ...
    ijod.getPars(base, options, function (err, pars) {
      ...
      async.forEachLimit(Object.keys(pars), 10, function (idh, cbPars) {
        ...
        options.pars.push(incrementParallel(selectParallel(pars[idh].pars,
  ...
  async.forEachLimit(Object.keys(friends), 10, function (pid, cbFriends) {
    ...
    async.waterfall([
      function (cb) { // handle indexing the related ids
        ...
        if (oe.url) {
          options.pars.push(parallelFromBytes([
  ...
  Object.keys(apps).forEach(function (app) {
    ...
    Object.keys(apps[app].accounts).forEach(function (account) {
      ...
      Object.keys(ids).forEach(function (id) {
        var index = {
          idr: base + '#' + encodeURIComponent(id),
          account: account,
          data: {},
          via: idr.toString(entry.idr)
        };
