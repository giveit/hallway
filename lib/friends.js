...
exports.parallels = function (entry) {
  ...
  if (oe.title) {
    ...
    ret.push(friendsUtil.parallelFromBytes([
      dMap.parallelType('first'),
      friendsUtil.str2sort(name.first),
      friendsUtil.str2num(name.first, 2)
    ]));

    ret.push(friendsUtil.parallelFromBytes([
      dMap.parallelType('last'),
      friendsUtil.str2sort(name.last),
      friendsUtil.str2num(name.last, 2)
    ]));
  }
  ...
  if (oe.email) {
    ret.push(friendsUtil.parallelFromBytes([
      dMap.parallelType('email'),
      friendsUtil.str2num(oe.email, 3)
    ]));
  }
  ...
  if (oe.phone) {
    ...
    var phone = friendsUtil.phoneHome(oe.phone);

    ret.push(friendsUtil.parallelFromBytes([
      dMap.parallelType('phone'),
      friendsUtil.str2num(phone, 3)
    ]));
  } else if (oe.handle) {
    // alternatively, any handle
    // TODO, maybe if no handle but there is email and the email is @gmail
    // @yahoo etc, use the username part?
    ret.push(friendsUtil.parallelFromBytes([dMap.parallelType('handle'),
      friendsUtil.str2num(oe.handle, 3)]));
  }
...
exports.versionPump = function (cset, auth, cbDone) {
  var ndx = {};
  var ver = friendsUtil.reversion(auth);

  // XXX: Dead code? ndx isn't used after this.
...
exports.bump = function (cset, auth, cbDone) {
  ...
  async.series([
    function (cb) {
      friendsUtil.indexMe(auth, self, cb);
    },
    function (cb) {
      friendsUtil.friendex(auth, pids, cb);
    },
    function (cb) { // device contacts are special, per-app
      if (deviceId) friendsUtil.devicePeers(deviceId[1], deviceId[2], pids, cb);
      else friendsUtil.peerCheck(auth, pids, cb);
    },
    function (cb) {
      friendsUtil.interactive(auth, iact, cb);
    }
  ], function () {
    cbDone(null, cset);
  });
  ...
  async.forEach(bases, function (base, cbBase) {
      ...
      ijod.getPars(base, (options.ace === base) ? { xids: true } : options,
        ...
        Object.keys(pars).forEach(function (id) {
          ...
          ids[id].pars.forEach(function (par) {
            ...
            // sake
            var type = friendsUtil.parallelType(par);
    ...
    function seek(id, friend) {
      ...
      ids[id].mergies.forEach(function (par) {
        ndx[friendsUtil.parallelType(par)][par].forEach(function (dup) {
          merge(friend, dup);
        });
      });
...
exports.validate = function (friend, options) {
  if (options.q &&
    !friendsUtil.checkPlease(options.q, ijod.qtext(friend), true)) {
    return false;
  }

  if (options.bio &&
    !friendsUtil.checkPlease(options.bio, friendsUtil.bioget(friend), false)) {
    return false;
  }
...
exports.generateToc = function (list, sort) {
  var toc = {
    "meta": {
      "length": list.length,
      "sort": sort
  ...
  // first || last
  var map = friendsUtil.tocmap(sort);
  var on = map.shift();
  on.start = 0;
